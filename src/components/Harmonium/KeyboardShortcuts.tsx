...
export function KeyboardShortcuts() {
  ...
  return (
    <>
      ...
      {isMounted && (
        <div
          className={`fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 transition-opacity duration-200 ${isVisible ? 'opacity-100' : 'opacity-0'}`}
          onClick={close}
        >
          <div
            className={`bg-card border border-border rounded-lg p-6 max-w-3xl w-full shadow-lg max-h-[90vh] overflow-y-auto transition-[opacity,transform] duration-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}
